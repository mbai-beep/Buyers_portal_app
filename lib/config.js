...
export function classifyDirectoryError(err) {
  ...
  if (m.includes('no such table') || code === 'SQLITE_UNKNOWN') {
    return { reason: 'schema_missing',
             hint: 'The employees table is not in the database this app is pointed at. Run the bootstrap endpoint.' };
  }
  if (m.includes('no such column')) {
    return { reason: 'schema_mismatch',
             hint: 'The employees table exists but its columns differ from the expected schema. Run the bootstrap endpoint.' };
  }
