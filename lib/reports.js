 ...
 *
 * Fabric is deliberately absent. It was specified as FabricShortName, but the
 * server answered "Invalid column name 'FabricShortName'" - so it either does
 * not exist on these views or is called something else. Rather than delete the
 * capability, it is off unless SQL_COL_FABRIC names the real column; set that
 * and every fabric breakdown comes back with no other change.
 */
export const GRAIN = {
  article: 'ArticleNo',
  category: 'CategoryShortName',
  supplier: 'SupplierAlias',
  ...(process.env.SQL_COL_FABRIC ? { fabric: process.env.SQL_COL_FABRIC } : {}),
};

export const hasGrain = (name) => Boolean(GRAIN[name]);
...
export async function totals(key, range = null) {
  ...
  const rows = await query(
    `SELECT CAST(SUM(ISNULL(${v.qty}, 0))   AS float) AS qty,
            ...
            ${hasGrain('fabric') ? `COUNT(DISTINCT ${GRAIN.fabric}) AS fabrics,` : ''}
            COUNT_BIG(*)                              AS rows
  ...
  return {
    ...
    ...(hasGrain('fabric') ? { fabrics: num(r.fabrics) } : {}),
    rows: num(r.rows),
  };
}
...
export async function articleLeaders({ from, to, limit = 50 }) {
  ...
  const [sold, stock] = await Promise.all([
    query(
      `SELECT TOP ${n} ${GRAIN.article} AS article,
              ...
              ${hasGrain('fabric') ? `MAX(${GRAIN.fabric}) AS fabric,` : ''}
  ...
  return sold.map((r) => {
    ...
    return {
      article: r.article, category: r.category, supplier: r.supplier,
      ...(hasGrain('fabric') ? { fabric: r.fabric } : {}),
...
export async function supplierDetail({ alias, from, to }) {
  ...
  return {
    ...
    articles: articles.map((r) => ({
      article: r.article, category: r.category,
      ...(hasGrain('fabric') ? { fabric: r.fabric } : {}),
    ...
    fabricAvailable: hasGrain('fabric'),
  };
}
