...
export async function homeTotals({ from, to }) {
  return cached(`home:${from}:${to}:${HINT}`, async () => {
    ...
    const [sold, soldFy, purchased, purchasedFy, returned, stock] = await Promise.all([
    ...
    return {
      ...
      suppliers: stock.suppliers || purchased.suppliers,
      designsInStock: stock.articles,
      designsBoughtThisYear: purchasedFy.articles,
    };
  });
}
