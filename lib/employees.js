...
export async function upsertEmployee({
  name, email, designation, department, desk, role, employeeCode, isActive,
}) {
  const e = normaliseEmail(email);
  await db().execute({
    sql: `INSERT INTO portal_users
            (name, email, designation, department, desk, role, employee_code, is_active)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    ...
    args: [name, e, designation ?? null, department ?? null, desk ?? null,
           role || 'buyer', employeeCode ?? null, isActive === false ? 0 : 1],
  });
