      ...
      if (!tableNames.includes('employees')) {
        ...
        if (missing.length) {
          ...
          const { rows } = await client.execute(
            ...
            'SUM(CASE WHEN password_hash IS NULL THEN 1 ELSE 0 END) AS never_signed_in FROM employees'
          );
          check.employees = {
          ...
          if (check.employees.active === 0) {
            ...
            check.detail = 'The employees table exists but holds no active rows. Run the bootstrap endpoint.';
            out.problems.push(check.detail);
          }
        }
      }
